...
class EmailVerificationService(
    ...
    fun registerEmail(userId: UUID, email: String): Mono<String> {
        ...
        return checkAndUpdateRateLimit(userId)
            .flatMap { userAccountRepository.findById(userId)
            ...
            .flatMap { userAccount ->
                ...
                if (userAccount.email != null) {
                    return@flatMap Mono.error<String>(
                        BusinessException(ErrorCode.EMAIL_UPDATE_NOT_ALLOWED)
                    )
                }
                    ...
                    .flatMap { exists ->
                        if (exists) {
                            ...
                            codeRepository.save(verificationCode)
                                ...
                                .doOnSuccess {
                                    logger.info { "Email verification initiated for user: $userId, email: $email" }
                                }
                        }
                    }
            }
    ...
    fun verifyEmail(userId: UUID, code: String): Mono<Unit> {
        return codeRepository.findById(code)
            ...
            .flatMap { verificationCode ->
                ...
                // Update user email
                userAccountRepository.findById(verificationCode.userId)
                    .switchIfEmpty(Mono.error(NotFoundException(ErrorCode.NOT_FOUND, "User not found")))
                    .flatMap { userAccount ->
                        // Check if email is still available
                        userAccountRepository.existsByEmail(verificationCode.email)
                            .flatMap { exists ->
    ...
    fun resendVerificationEmail(userId: UUID, email: String): Mono<Void> {
        // Check rate limit before proceeding
        return checkAndUpdateRateLimit(userId)
            .flatMap {
                userAccountRepository.findById(userId)
                    ...
                    .then(Mono.defer {
                        ...
                        codeRepository.save(verificationCode)
                            ...
                            .doOnSuccess {
                                logger.info { "Verification email resent for user: $userId, email: $email" }
                            }
                    })
                    }
            }
    ...
    private fun checkAndUpdateRateLimit(userId: UUID): Mono<Unit> {
        ...
        return rateLimitRepository.findById(userIdStr)
            .flatMap { rateLimit ->
                ...
                when {
                    !rateLimit.canRequest() -> {
                        ...
                        // Update rate limit
                        rateLimitRepository.save(rateLimit.incrementCount())
                            .thenReturn(Unit)
                            .doOnSuccess {
            ...
            .switchIfEmpty(
                ...
                rateLimitRepository.save(EmailVerificationRateLimit(userId = userIdStr))
                    .thenReturn(Unit)
                    .doOnSuccess {
